-- Oceny użytkowników: tabela + RLS + agregat RPC.
-- Do uruchomienia w projekcie katalogowym (ten, do którego łączy się
-- src/lib/catalogClient.ts) — tam mieszka auth.users i saved_activities.
...
CREATE POLICY "Users can view their own ratings"
  ON public.user_ratings FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own ratings"
  ON public.user_ratings FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own ratings"
  ON public.user_ratings FOR UPDATE TO authenticated
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete their own ratings"
  ON public.user_ratings FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
DROP TRIGGER IF EXISTS user_ratings_before_update ON public.user_ratings;
CREATE TRIGGER user_ratings_before_update
BEFORE UPDATE ON public.user_ratings
FOR EACH ROW EXECUTE FUNCTION public.user_ratings_touch_updated_at();

-- Agregat: tylko średnia i liczba ocen. Zero danych osobowych.
